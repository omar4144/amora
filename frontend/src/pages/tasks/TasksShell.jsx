...
import { LayoutDashboard, KanbanSquare, ListChecks, CheckSquare } from "lucide-react";

export default function TasksShell() {
    const tabs = [
        { to: "/tasks", icon: LayoutDashboard, label: "لوحة التحكم", end: true },
        { to: "/tasks/boards", icon: KanbanSquare, label: "اللوحات" },
        { to: "/tasks/my", icon: ListChecks, label: "مهامي" },
    ];
    return (
        <div data-testid="tasks-shell" className="pb-24 min-h-[100dvh]">
            <div className="px-4 pt-6 pb-3 border-b border-white/5 sticky top-0 bg-black/95 backdrop-blur-xl z-30">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h1 className="font-heading font-black text-lg leading-none">المهام</h1>
                            <p className="text-[10px] text-white/50 font-body">مركز إدارة المهام والمشاريع</p>
                        </div>
                    </div>
                </div>
                <div className="flex gap-1 overflow-x-auto -mx-4 px-4 scrollbar-none">
                    {tabs.map((t) => (
                        <NavLink key={t.to} to={t.to} end={t.end}
                            data-testid={`tasks-tab-${t.to.split('/').pop()||'dashboard'}`}
                            className={({ isActive }) =>
