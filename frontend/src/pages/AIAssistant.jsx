    ...
    return (
        <div className="p-6 pt-8 font-body pb-24" data-testid="ai-page">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4 pb-1">
                {TASKS.map((t) => (
                    <button key={t.id} data-testid={`task-${t.id}`} onClick={() => { setTask(t); setResult(""); }} className={`shrink-0 px-4 py-2 rounded-full text-xs font-heading font-bold whitespace-nowrap ${task.id === t.id ? "bg-[#D1795F] text-white" : "bg-[#141414] border border-[#262626]"}`}>{t.label}</button>
                ))}
            </div>

            <form onSubmit={run} className="mb-4">
                <textarea data-testid="ai-context" required value={ctx} onChange={(e) => setCtx(e.target.value)} placeholder={task.placeholder} rows={4} className="w-full bg-[#141414] border border-[#262626] rounded-2xl px-4 py-3 focus:border-[#D1795F] focus:outline-none resize-none mb-3" />
                <button data-testid="ai-submit" type="submit" disabled={loading} className="w-full bg-[#D1795F] text-white font-heading font-bold rounded-full py-3 disabled:opacity-50">
                    {loading ? "جارٍ التفكير..." : `اطلب: ${task.label}`}
                </button>
            </form>

            {result && (
                <div className="bg-[#141414] border border-[#D1795F]/30 rounded-2xl p-4 whitespace-pre-line text-sm leading-relaxed" data-testid="ai-result">
                    {result}
                </div>
            )}
        </div>
