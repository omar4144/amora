...
export default function Auth() {
    ...
    return (
        <div className="min-h-[100dvh] w-full max-w-md mx-auto bg-black text-white flex flex-col justify-center p-6 font-body relative overflow-hidden">
            <div className="absolute -top-20 -start-20 w-64 h-64 rounded-full bg-[#D1795F]/10 blur-3xl" />
            ...
            <div className="relative">
                ...
                <div className="flex gap-2 bg-neutral-900 p-1 rounded-full mb-8">
                    <button
                        ...
                        className={`flex-1 py-2 rounded-full font-heading font-bold text-sm transition-all ${
                            mode === "login" ? "bg-[#D1795F] text-white" : "text-neutral-400"
                        }`}
                    >
                        تسجيل الدخول
                    </button>
                    <button
                        ...
                        className={`flex-1 py-2 rounded-full font-heading font-bold text-sm transition-all ${
                            mode === "signup" ? "bg-[#D1795F] text-white" : "text-neutral-400"
                        }`}
                    >
                        إنشاء حساب
                    </button>
                ...
                <form onSubmit={submit} className="flex flex-col gap-4">
                    {mode === "signup" && step === 1 && (
                        <>
                            <input data-testid="input-name" placeholder="الاسم الكامل" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="bg-[#141414] border border-[#262626] rounded-xl px-4 py-3.5 text-white placeholder-neutral-500 focus:border-[#D1795F] focus:outline-none transition" />
                            <input data-testid="input-username" placeholder="اسم المستخدم (بالإنجليزية)" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value.toLowerCase().replace(/\s/g, "") })} required pattern="[a-z0-9_]{3,20}" className="bg-[#141414] border border-[#262626] rounded-xl px-4 py-3.5 text-white placeholder-neutral-500 focus:border-[#D1795F] focus:outline-none transition" />
                        </>
                    )}
                    {(mode === "login" || (mode === "signup" && step === 1)) && (
                        <>
                            <input data-testid="input-email" type="email" placeholder="البريد الإلكتروني" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required className="bg-[#141414] border border-[#262626] rounded-xl px-4 py-3.5 text-white placeholder-neutral-500 focus:border-[#D1795F] focus:outline-none transition" />
                            <input data-testid="input-password" type="password" placeholder="كلمة المرور" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required minLength={6} className="bg-[#141414] border border-[#262626] rounded-xl px-4 py-3.5 text-white placeholder-neutral-500 focus:border-[#D1795F] focus:outline-none transition" />
                        </>
                    )}
                    ...
                    {mode === "signup" && step === 2 && (
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2 max-h-[380px] overflow-y-auto no-scrollbar">
                                {ROLES.map((r) => (
                                    <button type="button" key={r.id} data-testid={`role-${r.id}`} onClick={() => setForm({ ...form, role: r.id })} className={`text-start px-3 py-3 rounded-xl border font-heading font-bold text-sm transition ${form.role === r.id ? "bg-[#D1795F] text-white border-[#D1795F]" : "bg-[#141414] border-[#262626] hover:border-white/30"}`}>{r.label}</button>
                                ))}
                            </div>
                        </div>
                    )}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {LOOKING_FOR.map((item) => {
                                    ...
                                    return (
                                        <button type="button" key={item} data-testid={`lf-${item}`} onClick={() => toggleLF(item)} className={`flex items-center justify-center gap-2 px-3 py-3 rounded-xl border font-heading font-bold text-sm transition ${active ? "bg-[#D1795F] text-white border-[#D1795F]" : "bg-[#141414] border-[#262626]"}`}>
                    ...
                    {mode === "signup" && step < 3 ? (
                        <button type="button" data-testid="next-step" onClick={() => setStep(step + 1)} disabled={step === 1 && (!form.email || !form.password || !form.name || !form.username)} className="bg-[#D1795F] text-white font-heading font-bold rounded-full py-3.5 hover:bg-[#B86648] transition active:scale-95 disabled:opacity-50 mt-2">
                            التالي →
                        </button>
                    ) : (
