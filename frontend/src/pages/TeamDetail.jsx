...
export default function TeamDetail() {
    ...
    return (
        <div className="p-6 pt-8 font-body pb-24" data-testid="team-page">
            ...
            <span className="text-xs bg-[#D1795F]/20 text-[#D1795F] px-2 py-1 rounded-full">{t.kind}</span>
            ...
            {!isOwner && (
                <button onClick={join} data-testid="join-team-btn" className={`w-full rounded-full py-3 font-heading font-bold mb-6 ${t.joined ? "bg-white/10" : "bg-[#D1795F] text-white"}`}>
            ...
            <div className="space-y-2">
                {t.members?.map((m) => (
                    <Link key={m.id} to={`/u/${m.user?.username}`} className="flex items-center gap-3 bg-[#141414] border border-[#262626] rounded-2xl p-3 hover:border-[#D1795F] transition" data-testid={`member-${m.user?.username}`}>
                        <div className="w-10 h-10 rounded-full bg-[#D1795F] flex items-center justify-center text-black font-heading font-black">{m.user?.name?.[0]}</div>
                        ...
                        {m.role === "owner" && <Crown className="w-4 h-4 text-[#D1795F]" />}
                    </Link>
                ))}
            </div>
        </div>
