    ...
    return (
        <div className="p-6 pt-16 font-body pb-24" data-testid="teams-page">
            ...
            <div className="space-y-3">
                {items.map((t) => (
                    <Link key={t.id} to={`/teams/${t.id}`} data-testid={`team-${t.id}`} className="block bg-[#141414] border border-[#262626] hover:border-[#D1795F] rounded-2xl p-4 transition">
            ...
            {show && (
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShow(false)}>
                    <form onSubmit={create} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-[#0A0A0A] border border-white/10 rounded-2xl p-6 space-y-3" data-testid="new-team-modal">
                        ...
                        <input required data-testid="team-name" placeholder="اسم الفريق" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="w-full bg-[#141414] border border-[#262626] rounded-xl px-4 py-3 focus:border-[#D1795F] focus:outline-none" />
                        <textarea required data-testid="team-desc" placeholder="ما الذي يميز فريقكم؟" rows={3} value={f.description} onChange={(e) => setF({ ...f, description: e.target.value })} className="w-full bg-[#141414] border border-[#262626] rounded-xl px-4 py-3 focus:border-[#D1795F] focus:outline-none resize-none" />
                        <div className="grid grid-cols-4 gap-2">
                            {KINDS.map((k) => <button key={k.id} type="button" onClick={() => setF({ ...f, kind: k.id })} className={`text-xs py-2 rounded-lg font-heading font-bold ${f.kind === k.id ? "bg-[#D1795F] text-white" : "bg-[#141414] border border-[#262626]"}`}>{k.label}</button>)}
                        </div>
                        <button data-testid="submit-team" type="submit" className="w-full bg-[#D1795F] text-white font-heading font-bold rounded-full py-3">إنشاء</button>
                    </form>
                </div>
            )}
        </div>
