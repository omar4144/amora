    ...
    return (
        <div className="p-6 pt-8 font-body pb-24" data-testid="incubator-page">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2"><Sprout className="w-6 h-6 text-[#D1795F]" /><h1 className="text-2xl font-heading font-black">الحاضنة</h1></div>
                <button data-testid="new-idea-btn" onClick={() => setShow(true)} className="bg-[#D1795F] text-white font-heading font-bold rounded-full px-4 py-2 text-sm flex items-center gap-1"><Plus className="w-4 h-4" /> فكرة</button>
            </div>
            ...
            {!selected && (
                <>
                    ...
                    <div className="space-y-3">
                        {ideas.map((i) => (
                            <button key={i.id} onClick={() => setSelected(i)} data-testid={`idea-${i.id}`} className="w-full text-start bg-[#141414] border border-[#262626] hover:border-[#D1795F] rounded-2xl p-4 transition">
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="font-heading font-bold">{i.title}</h3>
                                    <span className="text-[#D1795F] font-heading font-black">{i.overall_progress}%</span>
                                </div>
                                <div className="w-full h-1.5 bg-black rounded-full overflow-hidden">
                                    <div className="h-full bg-[#D1795F]" style={{ width: `${i.overall_progress}%` }} />
                                </div>
                            </button>
                        ))}
                    </div>
            ...
            {selected && (
                <div>
                    ...
                    <div className="text-3xl font-heading font-black text-[#D1795F] mb-4">{selected.overall_progress}%</div>
                    <div className="space-y-3">
                        {stages.map((s) => {
                            ...
                            return (
                                <div key={s.id} className="bg-[#141414] border border-[#262626] rounded-2xl p-4" data-testid={`stage-${s.id}`}>
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <span className="text-[#D1795F] font-heading font-black text-lg">{st.progress}%</span>
                                    </div>
                                    <input type="range" min="0" max="100" step="10" value={st.progress} onChange={(e) => updateStage(s.id, parseInt(e.target.value))} data-testid={`stage-slider-${s.id}`} className="w-full accent-[#D1795F]" />
                                </div>
                            );
                        })}
                    </div>
            ...
            {show && (
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShow(false)}>
                    <form onSubmit={create} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-[#0A0A0A] border border-white/10 rounded-2xl p-6 space-y-3">
                        ...
                        <input required data-testid="idea-title" placeholder="اسم الفكرة" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full bg-[#141414] border border-[#262626] rounded-xl px-4 py-3 focus:border-[#D1795F] focus:outline-none" />
                        <textarea required data-testid="idea-desc" placeholder="اشرح فكرتك..." rows={3} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full bg-[#141414] border border-[#262626] rounded-xl px-4 py-3 focus:border-[#D1795F] focus:outline-none resize-none" />
                        <button data-testid="submit-idea" type="submit" className="w-full bg-[#D1795F] text-white font-heading font-bold rounded-full py-3">ابدأ الرحلة</button>
                    </form>
                </div>
            )}
        </div>
